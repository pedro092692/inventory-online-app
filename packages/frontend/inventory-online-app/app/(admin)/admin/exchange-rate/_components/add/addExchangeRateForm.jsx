'use client'
import CreateExchangeRateAction from '@/app/lib/actions/createExchangeRate'
...
export default function AddExchangeRateForm() {
    ...
    const [state, formAction, isPending] = useActionState(CreateExchangeRateAction, initialState)

    return (
        <Form className={`${styles.form} shadow`} action={formAction}>
            <Container
            ...
            <Button role="submit" type="secondary" disabled={isPending}>
                ...
                {isPending ? 'Guardando...' : 'Registrar tasa'}
            </Button>
        </Form>
    )
}
