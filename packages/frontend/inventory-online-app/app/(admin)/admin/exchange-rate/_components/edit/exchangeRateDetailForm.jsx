...
import styles from '@/app/(admin)/admin/exchange-rate/_components/exchangeRate.module.css'
import InputStyles from '@/app/ui/form/input/input.module.css'
import { Container } from '@/app/ui/utils/container'
import { Icon } from '@/app/ui/utils/icons/icons'
import UpdateExchangeRateAction from '@/app/lib/actions/updateExchangeRate'
import { OvalLoader } from '@/app/ui/loader/spinner'
import { useActionState, useState, useEffect, useRef } from 'react'

export default function ExchangeRateDetailForm({data}) {
    const initialState = { message: null, inputs: { value: data?.value }, errors: {} }
    const updateRate = UpdateExchangeRateAction.bind(null, data?.id)
    const [state, formAction, isPending] = useActionState(updateRate, initialState)
            ...
            {data &&
                <Form className={`${styles.formview} shadow`} action={formAction}>
                    ...
                    <Button role="submit" type="secondary" disabled={isPending} style={{cursor: 'pointer'}}>
                        ...
                        {isPending ? 'Guardando...' : 'Editar tasa'}
                    </Button>
                </Form>
            }
        </>
