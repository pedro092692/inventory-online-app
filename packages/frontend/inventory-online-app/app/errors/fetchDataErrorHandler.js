 ...
 */
const errorHandler = async (fn, setLoading = null, setErrorMsg = null, msg = '') => {
    try {
        return await fn()
    } catch (error) {
        const status = error.status || error.response?.status
        const message = error.message || error.response?.data?.message || error.response?.data?.errors ||  'Error inesperado'
